...
/*        { run_command,     "| %s ",    "sensors | grep 'dell_smm-isa-0000' -A 5 | grep 'temp1:' | awk '{print $2}' | cut -d '+' -f2" },*/
        { run_command,     "| %s ",     "sensors coretemp-isa-0000 | grep 'Package id 0:' | awk '{print $4}' | cut -d '+' -f2" },
        { ram_used,        "| %s ]",    NULL },
//        { battery_perc,    "| %s%% ]", "BAT0" },
        { run_command,     "[ %s ]",    "TZ=Europe/Warsaw date +\"%a %d %b | %H:%M\"" },
//       { run_command,     "[ %s ]",    "TZ=Europe/Warsaw date +\"%a %d.%m | %H:%M\"" },
//        { run_command,     "[ %s ]",   "TZ=Europe/Warsaw date +\"%a %d %b %H:%M\"" },
//        { datetime,        " %s ]",    "%H:%M" },
};
