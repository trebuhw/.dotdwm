...
echo "  2) Tryb pracy (Enter = wykonanie od razu):"
echo "       [1] Podgląd (dry run) - pokazuje co by się stało, NIC nie zmienia."
echo "       [0] Wykonanie - zmienia nazwy plików naprawdę."
echo "  3) Opcjonalny własny fragment nazwy między datą a numerem"
echo "     (Enter aby pominąć), np. RRRR-MM-DD Kamila 0001.ext"
echo "======================================================================"
echo ""
...
while [ "$DRY_RUN" != "0" ] && [ "$DRY_RUN" != "1" ]; do
    read -r -p "Wybierz tryb [1 = podgląd / 0 = wykonaj zmiany] (Enter = 0): " DRY_RUN
    if [ -z "$DRY_RUN" ]; then
        DRY_RUN="0"
    fi
...
if [ "$DRY_RUN" = "1" ]; then
    ...
    read -r -p "Na pewno kontynuować? (T/n, Enter = tak): " confirm
...
# --- Krok 3: opcjonalny własny fragment nazwy ---
read -r -p "Dodatkowy fragment nazwy między datą a numerem (opcjonalnie, Enter aby pominąć): " LABEL
if [ -n "$LABEL" ]; then
    # usuwamy znaki, które mogłyby namieszać w nazwach plików (/ i cudzysłowy)
    LABEL=$(printf '%s' "$LABEL" | tr -d '/"'"'"'')
    echo "Nazwy będą wyglądać np. tak: RRRR-MM-DD $LABEL 0001.ext"
fi
...

{
    ...
    echo "Dodatkowy fragment nazwy: $([ -n "$LABEL" ] && echo "$LABEL" || echo "(brak)")"
    echo ""
} > "$LOG_FILE"
...
while IFS=$'\t' read -r date fname; do
    ...
    ext="${fname##*.}"
    if [ -n "$LABEL" ]; then
        newname=$(printf "%s %s %04d.%s" "$date" "$LABEL" "$counter" "$ext")
    else
