...
struct Monitor {
	...
	int smartmode;        /* 1: smart gaps/borders mode */
...
static void resizemfact(const Arg *arg);
...
static void togglesmartmode(const Arg *arg);
...
createmon(void)
{
	...
	m->topbar = topbar;
	m->gappx = gappx;
	m->smartmode = 1;   /* 1 = smart mode (no gaps/borders), 0 = normal */
	m->lt[0] = &layouts[0];
...
/* resizemfact: zmiana szerokosci okna przeciaganiem myszy (mfact) w trybie
 * kafelkowym; dla okien plywajacych zachowuje sie jak standardowy resizemouse */
void
resizemfact(const Arg *arg)
{
	int ocx, ocy, nw, nh;
	Client *c;
	Monitor *m;
	...
	/* okno plywajace: zachowaj sie jak resizemouse */
	if (c->isfloating || !selmon->lt[selmon->sellt]->arrange) {
		restack(selmon);
		ocx = c->x;
		ocy = c->y;
	...
	/* tryb kafelkowy: zmien mfact przez przeciaganie w poziomie */
	restack(selmon);
	if (XGrabPointer(dpy, root, False, MOUSEMASK, GrabModeAsync, GrabModeAsync,
		None, cursor[CurResize]->cursor, CurrentTime) != GrabSuccess)
		return;
	...
	do {
		...
		case MotionNotify:
			...
			if (startx < 0)
				...
				float delta = (float)(ev.xmotion.x_root - startx) / selmon->ww;
				float f = startmfact + delta;
				if (f >= 0.05 && f <= 0.95) {
					selmon->mfact = f;
					arrange(selmon);
				}
...
void
togglesmartmode(const Arg *arg)
{
	selmon->smartmode = !selmon->smartmode;
	arrange(selmon);
}
...
tile(Monitor *m)
{
	...
	/* smartmode: no gaps ever; no borders for any window */
	if (m->smartmode) {
		gap = 0;
		for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
			c->bw = 0;
...
updatewmdesktop(Client *c)
{
	...
	/* EWMH _NET_WM_DESKTOP nie ma jak wyrazic maski bitowej tagow dwm 1:1,
	 * wiec eksportujemy indeks pierwszego (najnizszego) tagu, na ktorym
	 * dane okno faktycznie jest - to jest to, czego potrzebuja narzedzia
	 * typu wmctrl/xdotool oraz nasz rofi-window-switcher. */
	for (i = 0; i < LENGTH(tags) && !((1 << i) & c->tags); i++);
	desktop = (i < LENGTH(tags)) ? i : 0;
